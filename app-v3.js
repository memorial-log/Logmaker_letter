...
function hasPreviewOnlyImageView(){var box=document.createElement('div');box.innerHTML=editor.innerHTML;return Array.prototype.some.call(box.querySelectorAll('.editor-image-block,.body-image'),function(fig){return fig.dataset.blank!=='true'&&(clampNumber(fig.dataset.zoom,100,300,100)!==100||clampNumber(fig.dataset.focusX,0,100,50)!==50||clampNumber(fig.dataset.focusY,0,100,50)!==50);});}
...
async function copy(button){
 ...
 if(copied){flashButtonLabel(button,'복사 완료');text('saveState',hasPreviewOnlyImageView()?'HTML 복사 완료 · 본문 이미지 확대/초점은 미리보기 전용':'HTML 복사 완료');}
...
function httpsCssUrlsOnly(value){var safe=true;String(value).replace(/url\(\s*(['"]?)([^'")]*)\s*\)/gi,function(match,quote,url){if(!/^https:\/\//i.test(String(url).trim()))safe=false;return match;});return safe;}
...
function makeArcaBodySafe(box,allowLocalImages){var indent=activeIndent(),blocked=['position','z-index','overflow','gap','transform','transform-origin','opacity','filter','backdrop-filter','clip-path','mask','-webkit-mask','mix-blend-mode','background-blend-mode','isolation','outline','outline-offset','writing-mode','pointer-events','user-select','resize','content','animation','transition','shape-outside'];
 ...
 box.querySelectorAll('figure.body-image').forEach(function(fig){
  ...
  addCaption(top,true);
  if(/^https:\/\//i.test(src)||(allowLocalImages&&/^data:image\//i.test(src))){var cleanImg=document.createElement('img');cleanImg.src=src;cleanImg.alt=img.getAttribute('alt')||'';cleanImg.referrerPolicy='no-referrer';cleanImg.style.cssText=imageSizeCss(fig)+'margin:0;border:0;';wrapper.appendChild(cleanImg);}
